...
as
    select  troop.id,troop.classId,troop.loadoutId,troop.skinId,
            GREATEST ((troopClass.maxHp     + ((troopClass.maxHp     * sum(modifier.maxHp    ))/100)),0.001) as "MAXHP",
            GREATEST ((troopClass.dmg       + ((troopClass.dmg       * sum(modifier.dmg      ))/100)),0.001) as "DMG",
            GREATEST ((troopClass.atkRange  + ((troopClass.atkRange  * sum(modifier.atkRange ))/100)),0) as "ATKRANGE",
            GREATEST ((troopClass.moveRange + ((troopClass.moveRange * sum(modifier.moveRange))/100)),0) as "MOVERANGE" from
